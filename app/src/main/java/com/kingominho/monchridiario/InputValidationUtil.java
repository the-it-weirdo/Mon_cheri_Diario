...
public class InputValidationUtil {
    ...
    final int SUCCESS_CODE_PASSWORDS_MATCH = 2;
    final int SUCCESS_CODE_REGEX_MATCH = 1;
    final int ERROR_CODE_EMPTY_STRING = -1;
    final int ERROR_CODE_REGEX_NO_MATCH = -2;
    final int ERROR_CODE_PASSWORD_NO_MATCH = -3;
    final String ERROR_MESSAGE_INVALID_PASSWORD = "Please enter a valid password." +
    ...
    public int validatePassword(String passPhrase) {
        if (passPhrase.trim().isEmpty()) {
            return this.ERROR_CODE_EMPTY_STRING;
        } else {
            ...
            if (!matcherPassword.matches()) {
                return this.ERROR_CODE_REGEX_NO_MATCH;
            } else {
                return this.SUCCESS_CODE_REGEX_MATCH;
            }
        }
    }
    ...
    public int confirmPassword(String passPhrase, String confirmedPassPhrase) {
        ...
        if (passPhraseResult == this.SUCCESS_CODE_REGEX_MATCH) {
            if (confirmedPassPhraseResult == this.SUCCESS_CODE_REGEX_MATCH) {
                if (passPhrase.compareTo(confirmedPassPhrase) == 0) {
                    return this.SUCCESS_CODE_PASSWORDS_MATCH;
                } else {
                    return this.ERROR_CODE_PASSWORD_NO_MATCH;
                }
            } else {
